...
export class SyncProcessor extends WorkerHost {
  ...
  async process(job: Job<SyncJobData>): Promise<void> {
    this.logger.log(
      `sync job start jobId=${job.id} repositoryId=${job.data.repositoryId} userId=${job.data.userId}`,
    );

    // placeholder — real implementation wired in T-2.2
    await Promise.resolve();

    this.logger.log(`sync job finish jobId=${job.id} repositoryId=${job.data.repositoryId}`);
  }
